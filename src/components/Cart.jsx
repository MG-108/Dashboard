    ...
    <div className="bg-half-transparent w-full fixed nav-item top-0 right-0 ">
      <section className="float-right h-screen  duration-1000 ease-in-out dark:text-gray-200 transition-all dark:bg-[#484B52] bg-white md:w-400 p-8">
        ...
        {cartData?.map((item, index) => (
          <div key={index}>
            <section className="flex items-center leading-8 gap-5 border-b-1 border-color dark:border-gray-600 p-4">
              <img className="rounded-lg h-80 w-24" src={item.image} alt="" />
              <div>
                <p className="font-semibold">{item.name}</p>
                <p className="txet-gray-600 dark:text-gray-400 text-sm font-semibold">
                  {item.category}
                </p>
                <div className="flex gap-4 mt-2 items-center">
                  ...
                  <div className="flex items-center border-1 border-r-0 border-color rounded">
                    ...
                    <p className="p-2 border-r-1 border-color dark:border-gray-600 text-yellow-600">
                      0
                    </p>
